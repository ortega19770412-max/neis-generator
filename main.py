...
def pad_to_min_bytes(text: str, min_bytes: int = MIN_BYTES) -> str:
    tails = [
        " 또한 신체적 폭력뿐 아니라 정신적 피해, 재산적 피해, 정보통신망을 이용한 피해까지 함께 이해하는 계기가 됨.",
        " 아울러 사례를 살펴보며 폭력의 다양한 양상을 인식하고, 서로를 존중하는 태도의 중요성을 되새기는 시간이 됨.",
...
<body>
  <div class="card">
    ...
    <div class="hint">출력은 항상 “활동명(날짜)을 통해” 형식으로 시작합니다.</div>
...
반드시 아래 조건을 지켜라.
- 출력은 1문장만 작성
- 문장은 반드시 "{prefix}"로 시작할 것
- 활동명이 반드시 앞부분에 포함되어야 함
- 활동내용이 키워드형이면 자연스럽게 풀어쓸 것
- 학교폭력은 신체적 폭력, 정신적 피해, 재산적 피해, 정보통신망을 이용한 피해를 모두 포함해야 함
- 공식적이고 자연스러운 문체로 작성할 것
- 이미 종결형 어미로 끝난 문장에는 종결형 어미를 또 붙이지 말 것
- 문장 끝은 명사형 종결어미로 마칠 것
- 어색한 종결(예: 됨됨, 었음됨) 금지
- 200바이트 이상 700바이트 이하
- 결과만 출력
...
출력 예시:
{prefix} 사례를 살펴보고 토의와 역할극에 참여하며, 학교폭력의 유형과 예방의 중요성을 이해하는 계기가 됨.
"""
